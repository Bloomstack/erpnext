...
def get_batch_no(doctype, txt, searchfield, start, page_len, filters):
	...
	if args.get('warehouse'):
		return frappe.db.sql("""select sle.batch_no, round(sum(sle.actual_qty),2), sle.stock_uom,
		...
		return frappe.db.sql("""
			select batch.name, concat('MFG-', batch.manufacturing_date), concat('EXP-', batch.expiry_date)
				from `tabBatch` batch
					{package_tag}
			...
			limit %(start)s, %(page_len)s""".format(
				cond=cond,
				package_tag="inner join `tabPackage Tag` package_tag where batch.name=package_tag.batch_no" if filters.get("package_tag") else "",
				package_tag_query="and package_tag.batch_no={0}".format(frappe.db.escape(filters.get("package_tag"))) if filters.get("package_tag") else "",
				match_conditions=get_match_cond(doctype)
			), args)
